...
class TEEInterface:
    """
    Abstraction over platform TEE:
    - Android: Android Keystore (StrongBox / TEE-backed)
    - iOS: Secure Enclave (CryptoKit / SecKey)
    - Windows: TPM 2.0 via Windows Hello
    - Linux: PKCS#11 / TPM2-TSS

    In production, replace stub methods with platform SDK calls.
    """

    def __init__(self):
        # For demo: use software crypto (replace with real TEE SDK calls)
        self._key_store: Dict[str, bytes] = {}

    def generate_credential_key(self, credential_id: str) -> bytes:
        """Generate an ES256 key pair inside TEE. Returns public key bytes (COSE)."""
        # PRODUCTION: call android.security.keystore.KeyPairGenerator
        #             or SecKeyCreateRandomKey on iOS
        import secrets
        key_material = secrets.token_bytes(32)
        self._key_store[credential_id] = key_material
        # Return mock COSE_Key structure
        return json.dumps({
            "kty": 2,  # EC2
            "alg": -7,  # ES256
            "crv": 1,  # P-256
    ...
    def sign_assertion(self, credential_id: str, data: bytes) -> bytes:
        """Sign data inside TEE. Private key never leaves the secure boundary."""
        key = self._key_store.get(credential_id, b"fallback_key")
        # PRODUCTION: Signature.getInstance("SHA256withECDSA") in Android Keystore
        import hmac
        sig = hmac.new(key, data, digestmod=hashlib.sha256).digest()
        return sig
    ...
    def get_attestation(self, credential_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "attStmt": {
                ...
                "x5c": []  # Certificate chain from TEE vendor
            }
        }
